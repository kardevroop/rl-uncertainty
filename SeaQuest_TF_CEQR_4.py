...
# without percentiles
# uncertainty in replay buffer
# conda deactivate
# conda activate TF_Torch_RL
# cd /home/sr8685/
# python /home/sr8685/RL/CEQR-DQN/SeaQuest_TF_CEQR_4.py
...
file_name = '-TF-SeaQuest-CEQR-UncBuffer'
...
def mvn_act(state):
    ...
    # print(f"\nalpha: {alpha.shape}")
    # print(f"\nalpha_bar: {alpha_bar.shape}")
    # print(f"\nbeta_bar: {beta_bar}")
    # print(f"mu: {mu}")
    # print(f"v: {v}")
    # print(f"\nalpha: {alpha}")
    # print(f"alpha_bar - 1: {(alpha_bar - 1)}")
    ...
    u_ep = tf.sqrt((beta_bar + adam_epsilon / (v_bar*(alpha_bar - 1 + adam_epsilon))))
    u_al = tf.sqrt(beta_bar + adam_epsilon / (alpha_bar - 1 + adam_epsilon))
    # print(f"aleatoric: {aleatoric}")
    ...
    return action, u_al[action], u_ep[action]
...
for t in range(timesteps):
    ...
    is_training_ready = len(memory) >= replay_start_size

    print(f"\nTimestep: {t}; is_training_ready: {is_training_ready}")
    # print(f"state: {tf.cast(state, dtype=tf.float32)}")
    action, u_al, u_ep = mvn_act(tf.cast(state, dtype=tf.float32));
    # print(f"\naction: {action}; u_al: {u_al}; u_ep: {u_ep}\n")
    ...
    if (u_al < 1 or u_ep < 1):
        ...
        print("Action remembered")
    else:
        print("Action not remembered")
